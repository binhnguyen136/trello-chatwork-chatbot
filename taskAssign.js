function main() {
    ...
    for (var z=0; z<lists.length; z++) {
      ...
      if (cards.length) {
        ...
        for(var i=0; i<cards.length; i++) {
          if (checkNewCard(cards[i].id) && cards[i].idMembers.length) {
  ...
  if (isSendMessages && count) {
    message += "------------ END TRELLO TASK ASSIGNED -------------";
...
function checkNewCard(id) {
  var cardCreatedAt = new Date(1000*parseInt(id.substring(0,8),16));
  var diff = (new Date()) - cardCreatedAt;
  var msec = diff;
...
function postToChatwork(message){
  ...
  var url = "https://api.chatwork.com/v2/rooms/" + "152515994" + "/messages"
